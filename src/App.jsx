...
import { 
  ...
  FileText, Home, User, Send, Bot, Search, Activity, ChevronLeft, Tag, Languages, Eye, EyeOff, Delete, Globe, AlertTriangle, ChevronDown, ChevronUp, Camera, Sparkles, Loader2, ChevronRight, Settings, ShieldCheck, Database, Info, CreditCard, ArrowRightLeft
} from "lucide-react";
...
function App() {
  ...
  const exportPDF = () => {
    ...
    doc.text(`Periode: ${startDate} s/d ${endDate}`, 14, 37);
    ...
    doc.setTextColor(14, 165, 233); doc.setFontSize(10); doc.text(`NET: Rp ${periodBalance.toLocaleString('id-ID')}`, 20, 84);

    let currentY = 105;
    wallets.forEach((wallet) => {
      const walletTransactions = filteredData.filter(t => t.walletId === wallet.id);
      if (walletTransactions.length > 0) {
        doc.setTextColor(30, 41, 59); doc.setFontSize(12); doc.text(`DOMPET: ${wallet.name.toUpperCase()}`, 14, currentY);
        autoTable(doc, {
          startY: currentY + 4,
          head: [['Tanggal', 'Kategori', 'Tipe', 'Nominal']],
          body: walletTransactions.map(tr => [new Date(tr.createdAt.seconds * 1000).toLocaleDateString('id-ID'), tr.category.toUpperCase(), tr.type.toUpperCase(), `Rp ${tr.amount.toLocaleString('id-ID')}`]),
          theme: 'striped', margin: { left: 14, right: 14 }, didDrawPage: (data) => { currentY = data.cursor.y; }
        });
        currentY = doc.lastAutoTable.finalY + 15;
      }
    });
    doc.save(`Laporan_FinansialKu.pdf`);
  ...
  return (
    <div className={`min-h-screen flex flex-col max-w-md mx-auto transition-colors duration-500 ${darkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <div className="flex-1 px-5 -mt-24 z-30 pb-32 overflow-y-auto no-scrollbar">
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
                <div className={`rounded-[2.5rem] p-8 shadow-2xl border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-50'}`}>
                    <div className="space-y-4">
                        ...
                        <div className="pt-5 border-t border-slate-100 flex justify-between items-center gap-2">
                            <span className="font-black text-sm italic opacity-60 shrink-0">{t.totalBalance}</span>
                            <div className="flex items-center gap-3"><span className="text-sky-500 font-black italic tracking-tighter text-3xl">{showBalance ? `Rp ${formatRupiah(stats.balance)}` : "Rp ••••••"}</span><button onClick={() => setShowBalance(!showBalance)} className="text-slate-300"><EyeOff size={18}/></button></div>
                        </div>
                    </div>
                </div>
                ...
                {/* HISTORY SECTION */}
                <div className="space-y-4"><div className="flex justify-between items-center px-2"><h3 className="font-black text-xs opacity-50 uppercase italic tracking-widest">{t.activity}</h3>{allTransactions.length > 3 && (<button onClick={() => setShowAllHistory(!showAllHistory)} className="text-[10px] font-black text-sky-500 uppercase flex items-center gap-1">{showAllHistory ? t.showLess : t.seeAll} {showAllHistory ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}</button>)}</div>
                    <div className="space-y-3">
                        {displayedTransactions.map(tr => (
                            <div key={tr.id} className={`p-4 rounded-[2rem] flex justify-between items-center border transition-all ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-50 shadow-sm'}`}>
                                <div className="flex items-center gap-4 min-w-0 flex-1"><div className={`w-10 h-10 flex-shrink-0 rounded-2xl flex items-center justify-center ${tr.type === 'income' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>{tr.type === 'income' ? <ArrowUpCircle size={20}/> : <ArrowDownCircle size={20}/>}</div>
                                    <div className="min-w-0"><p className="font-black text-sm text-sky-500 uppercase truncate">{tr.category}</p><p className="text-[8px] text-slate-400 font-bold uppercase truncate">{wallets.find(w => w.id === tr.walletId)?.name || 'Wallet'}</p></div>
                                </div>
                                <div className="flex items-center gap-3"><p className={`font-black text-sm whitespace-nowrap ${tr.type === 'income' ? 'text-green-500' : 'text-red-500'}`}>{showBalance ? `${tr.type === 'income' ? '+' : '-'}Rp ${formatRupiah(tr.amount)}` : "Rp ••••••"}</p>
                                    <button onClick={async () => {if(window.confirm("Hapus?")) await deleteDoc(doc(db, "transactions", tr.id))}} className="text-slate-300 hover:text-red-500 transition-all"><Trash2 size={16}/></button>
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        {activeTab === "wallet" && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300"><div className="flex justify-between items-center px-2"><h2 className="font-black italic text-xl uppercase tracking-tighter">My Wallets</h2><button onClick={() => setShowAddWallet(true)} className="bg-sky-500 text-white p-2 rounded-xl shadow-lg"><PlusCircle size={20}/></button></div>
                {wallets.map(w => (
                    <div key={w.id} className={`p-6 rounded-[2.5rem] border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white shadow-sm'}`}>
                        <div className="flex items-center justify-between"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-sky-500/10 text-sky-500 rounded-2xl flex items-center justify-center"><WalletIcon size={24}/></div>
                                <div><p className="font-black text-xs uppercase opacity-50">{w.name}</p><p className="font-black text-lg text-sky-500">Rp {formatRupiah(allTransactions.filter(t => t.walletId === w.id).reduce((a, b) => a + (b.type === 'income' ? Number(b.amount) : -Number(b.amount)), 0))}</p></div>
        ...
        {activeTab === "category" && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300"><div className="flex justify-between items-center px-2"><h2 className="font-black italic text-xl uppercase tracking-tighter">Category & Budget</h2></div>
                <div className={`p-8 rounded-[2.5rem] border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white shadow-sm'}`}>
                    <div className="space-y-3 mb-6"><input value={newCatName} onChange={(e) => setNewCatName(e.target.value)} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`} placeholder="Category Name..." />
                        <div className="flex gap-2"><input value={newCatLimit} onChange={(e) => setNewCatLimit(e.target.value)} type="number" className={`flex-1 p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`} placeholder="Limit Budget..." />
                            <button onClick={async () => {if(!newCatName) return; await addDoc(collection(db, "categories"), { name: newCatName, limit: Number(newCatLimit) || 0, userId: user.uid, createdAt: new Date() }); setNewCatName(""); setNewCatLimit("");}} className="bg-orange-500 text-white px-6 rounded-2xl active:scale-95 shadow-lg"><PlusCircle size={20}/></button>
                        </div>
                    </div>
                    <div className="space-y-3">{categories.map(c => (
                            <div key={c.id} className={`p-5 rounded-[2rem] border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`}><div className="flex justify-between items-center mb-1"><span className="text-[10px] font-black uppercase opacity-70 tracking-widest">{c.name}</span><button onClick={async () => await deleteDoc(doc(db, "categories", c.id))} className="text-red-500/30 hover:text-red-500"><Trash2 size={16}/></button></div>
        ...
        {activeTab === "profile" && (
            <div className="space-y-6 animate-in slide-in-from-right duration-500">
                <div className={`p-8 rounded-[2.5rem] border shadow-xl text-center relative overflow-hidden transition-all ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-50'}`}>
                    ...
                    <p className="text-slate-400 text-xs font-bold mb-6 tracking-widest uppercase">{user.email}</p>
                    <div className="grid grid-cols-2 gap-4 border-t pt-6 border-slate-100 dark:border-slate-700">
                        <div className="text-left"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Saldo</p><p className="font-black text-sm text-sky-500 italic">Rp {formatRupiah(stats.balance)}</p></div>
                        <div className="text-right"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Transaksi</p><p className="font-black text-sm text-sky-500 italic">{allTransactions.length} Tx</p></div>
                    </div>
                </div>
                <div className="space-y-4">
                    <div className="space-y-2"><p className="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{t.account}</p>
                        <div className={`rounded-[2.5rem] p-4 space-y-1 shadow-sm border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-50'}`}>
                            ...
                            <button onClick={() => setActiveTab("wallet")} className="w-full flex items-center justify-between p-3 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-2xl transition-all"><div className="flex items-center gap-4"><div className="w-10 h-10 bg-green-100 dark:bg-green-500/20 text-green-500 rounded-xl flex items-center justify-center"><CreditCard size={18}/></div><span className="text-xs font-black uppercase tracking-tight">Atur Wallet</span></div><ChevronRight size={16} className="text-slate-300"/></button>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in zoom-in duration-300">
          <div className={`w-full max-w-sm p-8 rounded-[3.5rem] shadow-2xl ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
             ...
             <div className="space-y-4">
                <input type="number" value={transferForm.amount} onChange={(e) => setTransferForm({...transferForm, amount: e.target.value})} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50'}`} placeholder="Nominal Rp" />
                <select value={transferForm.fromWalletId} onChange={(e) => setTransferForm({...transferForm, fromWalletId: e.target.value})} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50'}`}>
                    <option value="">Dari Dompet</option>
                    {wallets.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
                <select value={transferForm.toWalletId} onChange={(e) => setTransferForm({...transferForm, toWalletId: e.target.value})} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50'}`}>
                    <option value="">Ke Dompet</option>
                    {wallets.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                </select>
                <button onClick={handleTransfer} className="w-full bg-teal-500 text-white py-5 rounded-[2rem] font-black uppercase text-[10px] active:scale-95 transition-all">Konfirmasi Transfer</button>
             </div>
          </div>
        </div>
      )}

      {/* MODAL LAINNYA (SCANNER, EXPORT, WALLET) */}
      {showScanner && (
        <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-md flex items-center justify-center p-6">
          <div className={`w-full max-w-sm p-10 rounded-[3.5rem] text-center ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-8 italic font-black text-indigo-500"><h2>{t.scannerTitle}</h2><button onClick={() => setShowScanner(false)}><X/></button></div>
            <label className="w-full flex items-center justify-center gap-3 py-5 rounded-[2rem] bg-indigo-500 text-white font-black uppercase text-[10px] cursor-pointer active:scale-95 transition-all">
                {isScanning ? <Loader2 className="animate-spin" /> : <Camera />} {isScanning ? 'Scanning...' : 'Select Bill'}<input type="file" accept="image/*" className="hidden" onChange={handleScanFile} disabled={isScanning} />
            </label>
          </div>
        </div>
      )}
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6">
          <div className={`w-full max-w-sm p-10 rounded-[3.5rem] ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
             <div className="flex justify-between items-center mb-8 italic font-black text-sky-500"><h2>REPORT</h2><button onClick={() => setShowExportModal(false)}><X/></button></div>
             <div className="space-y-4">
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700' : 'bg-slate-50'}`} />
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700' : 'bg-slate-50'}`} />
                <button onClick={exportPDF} className="w-full bg-orange-500 text-white py-5 rounded-[2rem] font-black uppercase text-[10px]">{t.downloadBtn}</button>
             </div>
          </div>
        </div>
      )}
      ...
      {showAddWallet && (
        <div className="fixed inset-0 z-[110] bg-black/60 flex items-center justify-center p-6">
          <div className={`w-full max-w-sm p-10 rounded-[3.5rem] ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
             <div className="flex justify-between items-center mb-8 font-black text-sky-600"><h2>NEW WALLET</h2><button onClick={() => setShowAddWallet(false)}><X/></button></div>
             <input value={newWalletName} onChange={(e) => setNewWalletName(e.target.value)} className={`w-full p-5 rounded-[2rem] outline-none border mb-6 text-sm font-black ${darkMode ? 'bg-slate-700' : 'bg-slate-50'}`} placeholder="Name..." />
             <button onClick={() => {if(newWalletName){addDoc(collection(db, "wallets"), { name: newWalletName, userId: user.uid, createdAt: new Date() }); setNewWalletName(""); setShowAddWallet(false);}}} className="w-full bg-sky-500 text-white py-5 rounded-[2rem] font-black uppercase text-xs">Save</button>
          </div>
        </div>
      )}
      ...
      {showAddTransaction && (
        <div className={`fixed inset-0 z-[100] flex flex-col animate-in slide-in-from-bottom duration-300 ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}`}>
          <div className="flex items-center justify-between p-6"><button onClick={() => setShowAddTransaction(false)} className={`p-3 rounded-full ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`}><ChevronLeft size={20}/></button><h2 className="font-black text-lg text-sky-500 uppercase italic">Add {form.type.toUpperCase()}</h2><div className="w-10"></div></div>
          <div className="px-6 mb-8"><div className={`${darkMode ? 'bg-slate-800' : 'bg-slate-100'} p-1 rounded-3xl flex`}><button onClick={() => setForm({...form, type: 'expense'})} className={`flex-1 py-3 rounded-2xl text-xs font-black transition-all ${form.type === 'expense' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-400'}`}>EXPENSE</button><button onClick={() => setForm({...form, type: 'income'})} className={`flex-1 py-3 rounded-2xl text-xs font-black transition-all ${form.type === 'income' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-400'}`}>INCOME</button></div></div>
          <div className="flex flex-col items-center mb-6"><h1 className="text-5xl font-black tracking-tighter mb-1 text-sky-500">Rp {formatRupiah(form.amount)}</h1><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Confirm Amount</p></div>
          <div className="px-6 mb-4 flex gap-2 overflow-x-auto no-scrollbar py-2">{categories.map(c => (<button key={c.id} onClick={() => setForm({...form, category: c.name})} className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-[10px] font-black border transition-all ${form.category === c.name ? 'bg-sky-600 text-white border-sky-600 shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}><Tag size={14}/> {c.name.toUpperCase()}</button>))}</div>
          <div className="px-6 mb-8 flex gap-2 overflow-x-auto no-scrollbar">{wallets.map(w => (<button key={w.id} onClick={() => setForm({...form, walletId: w.id})} className={`px-5 py-3 rounded-2xl text-[10px] font-black border transition-all ${form.walletId === w.id ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}>{w.name.toUpperCase()}</button>))}</div>
          <div className="px-6 mb-6"><button onClick={handleSubmit} className="w-full bg-sky-500 text-white py-5 rounded-[2rem] font-black uppercase text-xs active:scale-95 transition-all">Save Transaction</button></div>
          <div className={`mt-auto grid grid-cols-3 gap-1 p-2 pb-12 ${darkMode ? 'bg-slate-800/50' : 'bg-slate-50'}`}>{[1, 2, 3, 4, 5, 6, 7, 8, 9, "00", 0].map(n => (<button key={n} type="button" onClick={() => handleNumpad(n.toString())} className="py-6 text-2xl font-black transition-all rounded-xl">{n}</button>))}<button type="button" onClick={() => handleNumpad("delete")} className="py-6 flex items-center justify-center text-red-500 active:scale-90"><Delete size={28}/></button></div>
        </div>
      )}
    </div>
  );
