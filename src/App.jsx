...
function App() {
    ...
    wallets.forEach((wallet) => {
      ...
      if (walletTransactions.length > 0) {
        ...
        autoTable(doc, {
          ...
          body: walletTransactions.map(tr => [new Date(tr.createdAt.seconds * 1000).toLocaleDateString('id-ID'), tr.category.toUpperCase(), tr.type === 'income' ? 'MASUK' : 'KELUAR', `Rp ${tr.amount.toLocaleString('id-ID')}`]),
          theme: 'striped', margin: { left: 14, right: 14 }, didDrawPage: (data) => { currentY = data.cursor.y; }
        });
        currentY = doc.lastAutoTable.finalY + 15;
      }
    });
    doc.save(`Laporan_FinansialKu_${startDate}.pdf`);
  ...
  return (
    <div className={`min-h-screen flex flex-col max-w-md mx-auto transition-colors duration-500 ${darkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-800'}`}>
        ...
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in zoom-in duration-300">
          <div className={`w-full max-w-sm p-8 rounded-[3.5rem] shadow-2xl ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
             ...
             <div className="space-y-4">
                ...
                <select value={transferForm.fromWalletId} onChange={(e) => setTransferForm({...transferForm, fromWalletId: e.target.value})} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <div className="space-y-1"><p className="text-[10px] font-black opacity-30 ml-2 uppercase">Tujuan Dompet</p>
                    <select value={transferForm.toWalletId} onChange={(e) => setTransferForm({...transferForm, toWalletId: e.target.value})} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`}>
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in zoom-in duration-300">
          <div className={`w-full max-w-sm p-10 rounded-[3.5rem] shadow-2xl ${darkMode ? 'bg-slate-800' : 'bg-white'}`}><div className="flex justify-between items-center mb-8 italic font-black text-sky-500 text-lg tracking-tighter"><h2>DOWNLOAD REPORT</h2><button onClick={() => setShowExportModal(false)}><X/></button></div>
             <div className="space-y-5"><div className="space-y-1"><p className="text-[10px] font-black opacity-30 ml-2">Mulai</p><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`} /></div>
                <div className="space-y-1"><p className="text-[10px] font-black opacity-30 ml-2">Sampai</p><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className={`w-full p-4 rounded-2xl outline-none text-xs font-black border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-100'}`} /></div>
                <button onClick={exportPDF} className="w-full bg-orange-500 text-white py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[10px] active:scale-95 flex items-center justify-center gap-2"><FileText size={16}/> {t.downloadBtn}</button>
             </div>
          </div>
        </div>
      )}
      ...
      {/* MODAL ADD TRANSACTION - IMPROVED SCREEN */}
      {showAddTransaction && (
        <div className={`fixed inset-0 z-[100] flex flex-col animate-in slide-in-from-bottom duration-300 ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}`}>
          ...
          <div className="flex items-center justify-between p-6 shrink-0">
            <button onClick={() => setShowAddTransaction(false)} className={`p-3 rounded-full ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
              <ChevronLeft size={20}/>
            </button>
            <h2 className="font-black text-lg text-sky-500 uppercase italic tracking-tighter">
              Add {form.type.toUpperCase()}
            </h2>
          ...
          <div className="px-6 mb-4 shrink-0">
            <div className={`${darkMode ? 'bg-slate-800' : 'bg-slate-100'} p-1 rounded-3xl flex`}>
              <button onClick={() => setForm({...form, type: 'expense'})} className={`flex-1 py-3 rounded-2xl text-xs font-black transition-all ${form.type === 'expense' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-400'}`}>EXPENSE</button>
              <button onClick={() => setForm({...form, type: 'income'})} className={`flex-1 py-3 rounded-2xl text-xs font-black transition-all ${form.type === 'income' ? 'bg-sky-500 text-white shadow-lg' : 'text-slate-400'}`}>INCOME</button>
            </div>
          </div>

          {/* Amount Display with Real-time Formatting (FIXED) */}
          <div className="flex flex-col items-center mb-4 shrink-0 px-6">
            <h1 className="text-4xl font-black tracking-tighter mb-1 text-sky-500 truncate w-full text-center">
                Rp {form.amount ? formatRupiah(form.amount) : "0"}
            </h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Confirm Amount</p>
          </div>

          {/* Selectable Area (Scrollable) */}
          <div className="flex-1 overflow-y-auto px-6 space-y-4 no-scrollbar">
            {/* Categories */}
            <div className="space-y-2">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Kategori</p>
              <div className="flex flex-wrap gap-2">
                {categories.map(c => (
                  <button key={c.id} onClick={() => setForm({...form, category: c.name})} className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[10px] font-black border transition-all ${form.category === c.name ? 'bg-sky-600 text-white border-sky-600 shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}>
                    <Tag size={12}/> {c.name.toUpperCase()}
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Wallets */}
            <div className="space-y-2">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Wallet</p>
              <div className="flex flex-wrap gap-2">
                {wallets.map(w => (
                  <button key={w.id} onClick={() => setForm({...form, walletId: w.id})} className={`px-4 py-2.5 rounded-2xl text-[10px] font-black border transition-all ${form.walletId === w.id ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}>
                    {w.name.toUpperCase()}
                  </button>
                ))}
              </div>
          ...
          {/* Keyboard & Save Button (Sticky Bottom) */}
          <div className={`shrink-0 ${darkMode ? 'bg-slate-800/80 backdrop-blur-md' : 'bg-slate-50'} border-t border-slate-200`}>
            ...
            <div className="grid grid-cols-3 gap-1 p-2">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9, "00", 0].map(n => (
                <button key={n} type="button" onClick={() => handleNumpad(n.toString())} className={`py-4 text-xl font-black active:bg-sky-500 active:text-white transition-all rounded-xl`}>{n}</button>
              ))}
              <button type="button" onClick={() => handleNumpad("delete")} className="py-4 flex items-center justify-center text-red-500 active:bg-red-500 active:text-white rounded-xl transition-all">
                <Delete size={24}/>
              </button>
            </div>
            {/* Big Save Button */}
            <div className="px-4 pb-6 pt-2">
              <button onClick={handleSubmit} className="w-full bg-sky-500 text-white py-5 rounded-[2rem] font-black uppercase text-xs active:scale-95 shadow-xl shadow-sky-500/20 transition-all flex items-center justify-center gap-3">
                <ShieldCheck size={18}/> {t.save}
              </button>
            </div>
          </div>
        </div>
